...
struct Request {
    ...
    SafeBuffer* safe_buffer; // Safe Buffer Address
};

struct Worker {
	RequestChannel* channel; // Control Channel Address
...
void* request_thread_function(void* arg) {
	...
	SafeBuffer *s = (SafeBuffer *) r->safe_buffer;
...
void* worker_thread_function(void* arg) {
    ...
    */
    ...
    RequestChannel* chan = w->channel;
    std::string s = chan->send_request("newthread");
    RequestChannel *workerChannel = new RequestChannel(s, RequestChannel::CLIENT_SIDE);
	
	// Safe Buffer
	SafeBuffer *sb = (SafeBuffer *) w->safe_buffer;

	// Histogram Container
	std::vector<int> *john_frequency_count = (std::vector<int> *) w->john_frequency_count;
	std::vector<int> *jane_frequency_count = (std::vector<int> *) w->jane_frequency_count;
	std::vector<int> *joe_frequency_count = (std::vector<int> *) w->joe_frequency_count;


	std::vector<int> john(10, 0);
	std::vector<int> jane(10, 0);
	std::vector<int> joe(10, 0);



    while(true) {
        std::string request = sb->retrieve_front();
        std::string response = workerChannel->send_request(request);

		// DEBUG
		//atomic_standard_output().println(request);	
		//atomic_standard_output().println(response);	

        if(request == "data John Smith") {
            john.at(stoi(response) / 10) += 1;
        }
        else if(request == "data Jane Smith") {            
            jane.at(stoi(response) / 10) += 1;
        }
        else if(request == "data Joe Smith") {            
            joe.at(stoi(response) / 10) += 1;
        }
        else if(request == "quit") {

        	pthread_mutex_lock(w->vector_lock);
        	for(int i = 0; i < john.size(); i++){
        		john_frequency_count->at(i) += (john.at(i));
        		jane_frequency_count->at(i) += (jane.at(i));
        		joe_frequency_count->at(i) += (joe.at(i));
        	}


            delete workerChannel;
        	pthread_mutex_unlock(w->vector_lock);
            break;
        }
    }
...
/*--------------------------------------------------------------------------*/
		...
		r1.safe_buffer = &request_buffer;      // SafeBuffer Address
		// Jane Smith
		r2.patient = "Jane Smith";
		r2.n = n;
		r2.safe_buffer = &request_buffer;
		// Joe Smith
		r3.patient = "Joe Smith";
		r3.n = n;
		r3.safe_buffer = &request_buffer;
		...
		// Populating Worker Threads
        std::cout << "Creating worker threads... ";

		pthread_t worker_thread[w];
		pthread_mutex_t lock; //Control access to the result vector
		
		// Standard Info for Worker Channels
		Worker worker_info;
		worker_info.channel = chan;
		worker_info.safe_buffer = &request_buffer; // Safe Buffer
		worker_info.john_frequency_count = &john_frequency_count;
		worker_info.jane_frequency_count = &jane_frequency_count;
		worker_info.joe_frequency_count = &joe_frequency_count;
		worker_info.vector_lock = &lock; // Vector Histogram Lock
...
/*--------------------------------------------------------------------------*/
		 ...
		 */
		 std::cout << "Timer: " << std::chrono::duration_cast<std::chrono::nanoseconds>(timer2-timer1).count() << " nanoseconds" << std::endl;
		 /*
		 	This is a good place to output your timing data.
		 */
